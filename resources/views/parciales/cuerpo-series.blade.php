                      ...
                      @if ($n->status === 'reserved')
                        <form method="POST" action="{{ route('series.anular', $n->id) }}"
                          ...
                          <input name="motivo" required minlength="10" maxlength="255"
                                 ...
                                 class="w-52 rounded border border-slate-300 text-xs">
    ...
    <div class="space-y-5">
      <form method="POST" action="{{ route('series.serie') }}"
        ...
        <label class="block text-xs text-slate-500">Tipo de comprobante
          <select name="document_type_id" required class="mt-1 w-full rounded border border-slate-300 text-sm">
            @foreach ($tipos as $t)
        ...
        <label class="flex items-center gap-2 text-xs text-slate-600">
          <input type="checkbox" name="is_active" value="1" checked class="rounded border border-slate-300"> Activa
        </label>
        <label class="flex items-center gap-2 text-xs text-slate-600">
          <input type="checkbox" name="is_default" value="1" class="rounded border border-slate-300">
      ...
      <form method="POST" action="{{ route('series.tipo') }}"
        ...
        <label class="flex items-center gap-2 text-xs text-slate-600">
          <input type="checkbox" name="requires_customer_tax_id" value="1" class="rounded border border-slate-300">
          Exige identificación fiscal del cliente
        </label>
        <label class="flex items-center gap-2 text-xs text-slate-600">
          <input type="checkbox" name="is_active" value="1" checked class="rounded border border-slate-300"> Activo
        </label>
