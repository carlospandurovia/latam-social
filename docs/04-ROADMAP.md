...
| 9.20 | ✅ **Cerrada (2026-08-31) — cada cosa en su sitio.** Reportado con estas palabras: *«me voy a configuración y cuando le doy clic en alguna me manda a uno de los menús»*. El desorden no era el orden: **nueve pantallas estaban dos veces** —sueltas en el menú y dentro de `/configuracion`—, así que entrar desde el panel dejaba al usuario sin saber dónde estaba. El menú queda en tres grupos y **una sola puerta** a la configuración (`DEC-233`); dentro, las áreas salen por bloques en un orden fijo que no depende de qué falte hoy (`DEC-234`); y los catálogos dejan de ser una etiqueta del menú para ser un área con portada y aviso propio (`DEC-235`). Cada pantalla de configuración abre con su miga de pan, y la lista de qué es configuración sale del registro de áreas, no de la plantilla. Sin migración y sin mover ninguna ruta |
| 9.21a | ✅ **Cerrada (2026-08-31) — la trastienda se muda a `/backoffice`.** Lo vio el negocio antes de escribir una línea de la landing: `/creadores` era la lista del admin y tiene que ser la puerta pública de los creadores. **149 rutas** se mudan con **una línea** —`docs/08` prohíbe escribir URLs a mano en las vistas, y esa regla es lo que hace gratis la mudanza (`DEC-236`)—; **21 se quedan fuera con su motivo escrito**, incluidas las dos trampas que comparten primer segmento con algo que sí se movió (`DEC-237`). Una prueba recorre todas las rutas con nombre para que plantar una en la raíz vuelva a ponerse roja. Las 390 URLs a mano que sí había —todas en las pruebas— quedan como `T-78` |
| 9.21b | ✅ **Cerrada (2026-08-31) — la puerta de la calle.** `/` habla a las **marcas** —el lado que paga— y `/creadores` a los creadores; las dos se enlazan entre sí (`DEC-238`). El texto **es un dato**: titular, bloques, botón y lo que sale en buscadores se editan desde `/backoffice/landing`, y cuelgan de la marca (`DEC-239`). La postulación pública escribe en `creator_applications`, que existía desde la Fase 2 con `source='landing'` **y sin nadie que escribiera** —la mesa puesta y sin puerta, como `document_series`—, con throttle y campo trampa en vez de CAPTCHA (`DEC-240`). Sin portada se va al acceso, no a un 404 (`DEC-241`). `verificar-pantallas.py` rechazó el atajo del compositor comodín, con razón |
| 9.21c | ✅ **Cerrada (2026-08-31) — el contacto que no se pierde.** El formulario de la portada de marcas crea un **prospecto**, no un correo: hoy el correo está en «log» y una instalación con el SMTP a medio configurar perdería cada contacto sin que nadie se entere (`DEC-242`). Misma forma que `creator_applications` a propósito, con columna puerta para que **sólo lo abierto** ocupe sitio — quien vuelve a escribir el año que viene es un contacto nuevo, no un duplicado (`DEC-243`). Descartar exige motivo y no borra; convertir exige decir **en qué cliente**, porque duplicar el alta sería tener dos sitios donde se crea un cliente (`DEC-244`). Bandeja en **Prospectos**, detrás de Clientes |
| 9.9a | ✅ **Cerrada (2026-08-31) — el impuesto es un dato.** `invoices` existe desde la Fase 2 con `tax_amount` y con la aritmética comprobada, pero **la tasa no existía en ninguna parte**: nadie sabía que el IGV es 18 %. Es un **periodo** con no solape, porque las tasas cambian y las facturas de antes no (`DEC-245`); se pregunta siempre por la fecha del documento y no se corrige — se publica la siguiente, cerrando la anterior el día antes (`DEC-246`). Sólo se siembra el IGV peruano, y desde el arranque de la instalación (`DEC-247`). Novena vez que se usa `Periodo` y la primera sin haber tenido que descubrir el defecto antes |
| 9.9b | ✅ **Cerrada (2026-09-01) — la factura que sale de una campaña.** `invoices` llevaba desde la Fase 2 con la mesa puesta y sin puerta; al abrirla salieron **cinco defectos que sólo se ven emitiendo de verdad**. Un borrador ya no gasta correlativo: el número se pide al emitir, así que descartarlo no deja hueco ante SUNAT (`DEC-248`). `ck_invoice_regime_country` tenía `'PE'` escrito dentro — se congela ahora el país del emisor y la regla de `DEC-047` se escribe sin nombrar ningún país, igual que el código del impuesto, que pasa a `countries.sales_tax_code` (`DEC-249`). `invoices` no tenía disparador de inmutabilidad aunque `ledger_entries` sí: ahora el núcleo fiscal se congela al emitir y la base comprueba que **las líneas sumen la cabecera** (`DEC-250`). Y la factura explica su propio importe sin salir de su fila; anular exige motivo y el número no vuelve a la serie (`DEC-251`). Queda `T-79`: los cuatro tipos peruanos siguen en un CHECK
| 9.9c | ✅ **Cerrada (2026-09-01) — el certificado con el que firma cada sociedad.** La bifurcación se puso sobre la mesa antes de escribir código y el negocio eligió **Greenter con certificado propio**, no un PSE (`DEC-252`); Greenter irá detrás de una frontera en 9.9d, porque es peruano y esto es para seis países. El certificado **no cabe en `integration_credentials`**: es la identidad de la sociedad, no la credencial de una conexión, y tiene vigencia propia escrita dentro del archivo. Se normaliza a PEM, se cifra, y **la contraseña del `.pfx` no se guarda** (`DEC-253`). Y el hallazgo que le va a pasar a quien despliegue: **OpenSSL 3 no abre los `.pfx` de SUNAT** —cifrado antiguo— y el error se parece a una contraseña equivocada, así que el mensaje trae la orden exacta para convertirlo (`DEC-254`). Columna puerta 34. De propina, `T-82`: un verificador nuevo que caza los CHECK que se caen en silencio ante un NULL, defecto cometido **cuatro veces**
| 9.17e | ✅ **Cerrada (2026-09-01) — la dirección de un proveedor no se teclea.** Reportado desde la instalación: *«¿por qué me pide la URL? si selecciono Pruebas debe ir al Beta»*. Tenía razón: los extremos de SUNAT son fijos y públicos, y pedirlos a mano es pedir que se teclee una constante — un carácter de más produce comprobantes que no llegan. `DEC-190` **del revés** (`DEC-255`). Ahora los declara el catálogo por (proveedor, entorno), sembrados y verificados contra `thegreenter/ws`, y la columna de la conexión se queda como excepción. `ck_iconn_url` pasa a disparador —la pregunta es cruzada— y de paso exige lo que faltaba: **un emisor electrónico va con una sociedad**, porque lleva su RUC (`DEC-256`). Y las otras dos preguntas del mismo formulario contestadas donde se hacen: dónde va la contraseña y dónde el certificado
| 9.17f | ✅ **Cerrada (2026-09-01) — integraciones por pestañas, y una activa por propósito.** Crítica del negocio a mi diseño, y **tenía más razón de la que creía**: al mirar quién vivía en `integration_connections` resultó que SMTP está en `.env` y Decolecta en `fx_sources`, así que el único inquilino real era SUNAT y el desplegable ofrecía crear conexiones que nadie lee (`DEC-257`). Encontró además un agujero real: la puerta era por proveedor, así que **dos emisores electrónicos podían estar activos a la vez**; ahora es por propósito, con `purpose_snapshot` para que la garantía sea un índice único y no un `COUNT(*)` sin bloqueo (`DEC-258`). Y la pestaña de facturación junta las tres cosas de emitir —conexión, certificado y folios— sin duplicar plantilla: las pantallas sueltas redirigen (`DEC-259`). Quedan `9.17g` (el correo a la base) y `9.17h` (los tipos de cambio)
| 9.17g | ✅ **Cerrada (2026-09-01) — la cuenta de correo deja de vivir en el servidor.** Era el ajuste que más dolía: el único que exigía entrar a la máquina para algo que cambia solo, y el que más caro sale tener mal —con el transporte en «log» nadie recibe nada y no hay ningún error—. Tabla propia (`DEC-257` aplicado), la contraseña por la puerta cifrada de `9.17d`, y **la precedencia escrita y a la vista**: manda la cuenta guardada; si no, el `.env` (`DEC-260`). Con botón de prueba que **escribe el resultado**. Y dos defectos de camino: la regla de «sabe a dónde llamar» miraba sólo a SUNAT —un servidor de correo no tiene URL— y una pestaña de `Communication` no cabe en un controlador de `Core`: se invierte con el registro `Pestanas` (`DEC-261`). Queda `9.17h`
| 9.17i | ✅ **Cerrada (2026-09-01) — la cara de las integraciones.** Crítica del negocio con la pantalla de otro producto suyo delante: *«esta pantalla no se compara a la que me hiciste para LOTEALO»*. Tenía razón y lo que faltaba se puede enumerar: no decía qué hace cada integración ni qué pasa si no se activa, no tenía estado ni forma de apagar, no llevaba a dónde conseguir lo que pide, y el mismo aviso salía **dos veces** sin decir a cuál se refería. Una sola plantilla de tarjeta para las cinco integraciones, con los avisos y las chapas repartidos por el controlador (`DEC-262`). El interruptor, porque `9.17g` guardaba y activaba en el mismo gesto sin camino de vuelta al `.env` (`DEC-263`), y el aviso del puerto cruzado con el cifrado (`DEC-264`). **El hallazgo grande salió de mirar la pantalla renderizada y no las pruebas:** Tailwind 4 quita el borde a todos los controles y el proyecto no usa `@tailwindcss/forms`, así que **130 campos en 20 pantallas** —incluidas la de acceso y las dos portadas públicas— llevaban desde siempre sin borde visible (`T-86`). Queda `9.17h` |
| 9.22b | ✅ **Cerrada (2026-09-02) — el correo no sale de una instalación que no es la de verdad.** La otra mitad de `DEC-029`, y la que de verdad se dispara sola: `9.17g` sacó la cuenta SMTP del `.env` —acierto— y con ello **una copia del volcado de producción trae dentro la cuenta buena**, así que un servidor de pruebas manda correos de verdad sin que nadie configure nada. Y `9.19b` escribe a **cada creador activo** al publicar términos: el destinatario es un tercero y un correo mandado no se retira. Aquí se **desvía** en vez de negarse, porque los fallos son distintos (`DEC-281`). Dos defectos salieron construyéndolo, y los dos habrían mentido en silencio: **«probar la cuenta» habría dicho que funcionaba** —el capturador nunca falla— estampando la fecha en la que el sistema afirma que esa cuenta servía; y **«desviado» se pintaba del rojo de «mal configurado»**, cuando en pruebas es el estado correcto (`DEC-282`). Cierra `T-88` con `verificar-salidas.py`, que reconoce al consumidor por pedir **la dirección o la clave** —el primer criterio, `status = 'active'`, dejaba fuera a los tipos de cambio, y el consumidor que no se detecta es justo el que el verificador existe para detectar— (`DEC-283`) |
| 9.22a | ✅ **Cerrada (2026-09-02) — desde aquí no se manda de verdad (`DEC-029`).** Llevaba desde el addendum marcada como *«la protección más importante»* y en estado PROPUESTA; **hasta `9.9e` no hacía falta**, porque el sistema no sabía mandar. El camino no exige ningún descuido raro: se restaura un volcado de producción en el servidor de pruebas —cosa que se hace todas las semanas—, alguien abre una factura que ya existía y pulsa «Mandar». **La identidad de la instalación vive en la máquina y no en la base**, y es la única excepción a `DEC-190`: puesta en la base viajaría dentro del volcado y la barrera se abriría sola justo cuando hace falta que esté cerrada (`DEC-278`). La barrera está en **la puerta** de las conexiones y no en cada consumidor, y dentro comprueba el entorno **antes** de buscar, para no descifrar el secreto antes de saber si se podía mirar (`DEC-279`). La anulación existe, se pinta en rojo y queda en la bitácora, y **se dice que le faltan dos de las tres cosas que `DEC-029` pedía** (`DEC-280`). De camino, **la cuarta aserción del proyecto que pasaba por el motivo equivocado**: se rompió la barrera a mano para ver qué se ponía rojo y la prueba de pantalla siguió verde —decía «desde aquí no se manda» porque no había enviador registrado, no por la barrera— |
| 9.9e | ✅ **Cerrada (2026-09-02) — el comprobante llega a la administración y la respuesta se guarda.** Cinco finales, porque exigen cinco arreglos distintos; la distinción que más cara sale confundir es **rechazado frente a error de red**, y por eso ante un rechazo la pantalla no pinta el botón de reintentar (`DEC-275`). Una tabla de INTENTOS y no sólo un estado: «¿está aceptada?» y «¿qué contestó SUNAT la primera vez?» son dos preguntas, y con sólo el estado cada reintento borra la respuesta anterior (`DEC-276`). **Se dice sin rodeos qué se probó aquí y qué se estrena en el servidor:** la costura está en `WsClientInterface`, donde separa de verdad «qué mando y qué entiendo» del cable (`DEC-277`). De camino salió que `external_status` llevaba desde la Fase 2 **sin vocabulario** —una aserción de `9.9b` guardaba «aceptada», inventado sobre la marcha porque nada lo impedía—: ahora `ck_invoice_external` lo cierra |
| 9.9f | ✅ **Cerrada (2026-09-02) — la factura congela su localidad y deja de suponer Perú (`T-87` y `T-79`).** Las dos deudas venían de `9.9b` y las destapó `9.9d`, que fue la primera iteración que **leyó** una factura para construir un documento oficial. El ubigeo y el distrito no estaban congelados aunque el comprobante los lleva dentro: mudar la sociedad habría hecho que el XML regenerado de una factura vieja fuese **distinto del que se emitió**, y los dos van firmados (`DEC-273`). Y los cuatro tipos peruanos salían de un CHECK: ahora la regla pregunta al catálogo del país del emisor, con disparador y no con foránea porque la pregunta es cruzada (`DEC-274`). **Escribir la prueba encontró que mi cambio había caído sólo en `borrador()` y no en `emitir()`** — las dos llamadas se parecen tanto que la sustitución acertó en la primera. Y una aserción de `T-79` pasaba por el motivo equivocado: la rechazaba «una factura sin líneas», no la regla que se estaba afirmando |
| 9.9d | ✅ **Cerrada (2026-09-02) — el XML firmado, detrás de una frontera.** `DEC-252` prometió que Greenter no se vería desde fuera; ahora **es una puerta**: capa propia en Deptrac, permitida sólo a `Finance/Emision/Peru/`, y comprobada metiendo un `use Greenter\…` prohibido para ver que Deptrac lo rechaza (`DEC-269`). Sale un UBL 2.1 firmado de verdad. **Generar de verdad destapó tres cosas que ninguna prueba de unidad habría visto:** la fecha del comprobante se iba un día atrás --`issue_date` es el día en la zona de la sociedad y construirla en UTC la renderizaba como el día anterior en hora de Lima--; `Greenter\See` crea un `SoapClient` en su constructor, así que usarla para *armar* obligaría a `ext-soap` en toda máquina; y la firma va en SHA-1 porque **SUNAT lo exige**, no por descuido de la librería. El XML vive en la base y no en `files`: es la prueba, no un archivo (`DEC-270`), con columna puerta 35 para que sólo haya uno vigente y el reemplazado no se borre (`DEC-271`). Y `Q-67` --la exportación de servicios con `tipoOperacion 0200`, que se identificó como supuesto legal sin revisar (§56)-- se cerró **el mismo día**: el negocio lo confirmó por experiencia propia y pasa a `DEC-272`. Queda `T-87` |
| 9.17j | ✅ **Cerrada (2026-09-01) — el sistema se entera de que le falta migrar (`T-84`).** De una mañana perdida en esta misma sesión: código de `9.17g` desplegado sin `migrate`, y un `SQLSTATE` en crudo con el mensaje de la regla anterior, que se llama igual y dice lo mismo que la nueva. **`T-83` estaba mal apuntado y medirlo lo demostró:** de 430 mensajes sólo uno se repite, y es la misma regla con dos puertas; lo que confundía eran dos VERSIONES de una regla, indistinguibles sobre un solo esquema. `App\Shared\Config\Esquema` compara los archivos con la tabla `migrations` en las dos direcciones y avisa en **todas** las pantallas del panel, con la orden exacta y sin cachear (`DEC-268`). De `T-83` sobrevive lo poco que valía: `verificar-mensajes.py` caza el mismo texto en dos tablas distintas, y hoy da cero |
| 9.17h | ✅ **Cerrada (2026-09-01) — la fuente de tipos de cambio deja de tener caja fuerte propia.** La última de las tres que el negocio pidió mover: con ella **ningún ajuste del sistema exige ya entrar a la máquina**. Lo que se pensó más fue qué NO se mueve: a la pestaña va la integración, y las tasas, las fuentes oficiales por par y el registro de traídas se quedan en su pantalla, con un enlace en cada dirección (`DEC-265`). `fx_sources` tenía cuatro columnas de credencial propias, más pobres que `integration_credentials`: **no versionaban, no revocaban y guardar una clave nueva borraba quién había puesto la anterior** (`DEC-266`). El criptograma se copia tal cual, así que la migración no descifra nada. Y salieron dos defectos de los de `T-69`: la dirección de Decolecta estaba en una constante de PHP **y además** en una columna que se podía teclear (`DEC-255` por segunda vez en tres iteraciones), y la precedencia era la CONTRARIA a la del correo en la misma pantalla, que es como se produce un «cambié la clave y no pasó nada» (`DEC-267`) |
