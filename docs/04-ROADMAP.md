...
| 9.17b | **El panel general de configuración** con badges de prioridad que generaliza los avisos de `9.16` y `9.17`, el **ubigeo** en `legal_entities` y las credenciales de cada API en un solo sitio (`DEC-190`). La razón social, el RUC, el domicilio y el representante legal ya se editan desde `4.5` en *Entidades legales* — son de la sociedad que factura, no de la marca |
| 9.18 | **Retención sobre el neto pactado y umbral de rentabilidad** (`DEC-191`): se pacta lo que el creador recibe, el sistema calcula el bruto con la tasa configurada y avisa si la participación no llega al margen mínimo |
| 9.19 | **Reaceptación de términos con plazo** (`DEC-193`): correo con enlace, 15 días, aviso en bandeja, recordatorio al entrar y solo lectura 30 días. Plazos configurables |
