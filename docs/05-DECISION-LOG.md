...
| **T-38** | 📋 **Las preguntas del creador antes de contestar una invitacion.** El roadmap las lista dentro de `7.6` y se dejaron fuera a proposito: un hilo de preguntas y respuestas es un mini modulo de mensajeria y meterlo habria duplicado el tamano de la iteracion (§2: treinta iteraciones pequenas antes que una gigantesca). Importa porque un creador con una duda que no puede preguntar **rechaza**, y ese rechazo entra en `decline_reason` como si fuera una opinion sobre la oferta. Mientras tanto el correo dice a quien responder | Cuando haya volumen real de invitaciones |
