...
| **T-67** | 📋 **Los archivos se guardan y no se pueden ver.** `Almacen` almacena documentos de identidad, evidencias de publicacion (`8.7`), comprobantes de pago (`9.7`) y de gasto (`9.10a`) --y **no existe ni una ruta que los devuelva**--. La pantalla de conciliacion acepta el comprobante y la de gastos dice «con comprobante», y no hay forma de abrirlo desde ninguna parte: una evidencia que nadie puede mirar no es una evidencia. No se arregla sobre la marcha porque la pregunta previa es **quien puede ver que archivo** --el creador el suyo, finanzas los comprobantes de pago, y el documento de identidad de un creador, quien?-- y eso es una decision de negocio, no una ruta. Salio al preguntar quien sirve los archivos, en la auditoria de `9.14b` | Antes de que alguien discuta un pago |
