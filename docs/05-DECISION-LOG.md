...
| **T-33** | 📋 **`deadline_offset_days` y `permanence_days` no tienen `CHECK`.** Los acota solo `GuardarRequisitoRequest` (0–365 y 0–3650), asi que un `permanence_days` de 100.000 entra hoy por cualquier `UPDATE` de mantenimiento o por una importacion, y con el un plazo que nadie puede cumplir. Se anota en vez de arreglarlo sobre la marcha porque 7.3 va a tocar esas mismas filas al partir los requisitos por mercado, y anadir dos `CHECK` ahora significa escribirlos dos veces | Iteracion 7.3 |
