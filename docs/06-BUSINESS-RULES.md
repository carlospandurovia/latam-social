...
| **BR-SEC-004** | Nunca se transmite una contraseña en texto claro por ningún canal. La activación de cuenta usa un enlace de un solo uso con expiración ≤72 h. **Implementada en 5.9/4.1** para los creadores y **cerrada del todo en 7.6b** (`T-36`, `DEC-126`): `usuarios:crear` ya no teclea la contraseña del equipo, y `usuarios:contrasena --enlace` tampoco al reponerla. Toda cuenta nace con un hash de 32 bytes aleatorios que nadie conoce. Queda **una excepción declarada**: reponer a mano sigue existiendo como cristal de emergencia para cuando el correo no sale (`Q-20`), y el comando avisa cada vez de que ése es el camino excepcional. | 🔴 |
...
| **BR-CREATOR-007** | Los cambios en datos fiscales, medios de pago o documento de identidad requieren aprobación interna antes de surtir efecto, y notifican al canal de contacto anterior. **Implementada en 3.6/3.8 (aprobación, con dos personas distintas) y 4.13 (notificación, `T-10`).** El aviso sale **al capturar** —mientras el cambio todavía se puede parar (`DEC-109`)— y **no lleva el dato dentro** (`DEC-110`). Falta el caso de *re-verificación de identidad*, que hoy no existe como flujo. | 🔴 |
| **BR-CREATOR-008** | La tarifa declarada por el creador es una referencia, no un compromiso. El precio vinculante es el **monto acordado congelado** en la participación de campaña. **Implementada en 7.5** (`DEC-104`, `tg_ccr_compromiso`): se fija al invitar —no se invita a nadie sin decirle cuánto se le paga— y se congela al aceptar. **Completada en 7.6** (`DEC-122`): faltaba la ventana de en medio. Entre que sale la invitación y el creador contesta, `agreed_amount` se podía mover, y el creador aceptaría una cifra que nunca vio. Ahora la invitación **copia** el importe y `tg_ccr_monto_con_invitacion` lo bloquea mientras haya una oferta encima; renegociar exige anular la invitación y mandar otra. | 🟠 |
...
| ID | Regla | Crit. |
|---|---|---|
| **BR-CONTENT-001** | Las versiones de entregables son inmutables. Una corrección crea una versión nueva; nunca se reemplaza un archivo. **Implementada en 8.1** (`uq_dv_number`, con el número calculado con la fila bloqueada) y **completada en 8.2**: `deliverables.approved_version_id` dice **cuál** es la buena, con clave ajena compuesta que garantiza que es de ese entregable y que impide borrarla mientras lo esté; y `tg_dv_entregable_abierto` impide añadir versiones sobre algo ya aprobado —el veto existía en el servicio desde 8.1 y la base no lo conocía (`T-47`)—. `deliverables.submitted_at` sigue guardando la **primera** entrega. | 🔴 |
| **BR-CONTENT-002** | Ningún contenido llega al cliente sin aprobación interna previa. **Implementada en 8.3** (`DEC-136`): aprobar es su propio permiso (`content.approve`), `ck_del_aprobador` exige que aprobado diga **quién**, y `tg_cvw_entregable_abierto` impide seguir revisando algo ya cerrado. Desde **8.2** dice además **qué versión** se aprobó (`ck_del_approved_version`), y volver atrás tiene un camino: reabrir con motivo y firma, sin borrar la aprobación anterior (`DEC-138`). **Cerrada en 8.5**: `tg_apl_version_aprobada` exige que la pieza esté `approved` **y** que la versión que se le enseña sea `approved_version_id`. El estado ya lo garantizaba; qué versión veía el cliente, no. | 🟠 |
| **BR-CONTENT-003** | El precio al cliente incluye **2 rondas de corrección**, **por entregable** y sólo las que pide el **cliente**. Es el valor por defecto de la campaña y puede cambiarse por campaña. Superarlo **bloquea** hasta que alguien con `content.extra_round` diga si se cobra o se absorbe, y queda firmado. **Implementada en 8.3** (`DEC-133`, `DEC-135`): `deliverables.revision_rounds_used`, `ck_cvw_over`. El contador estaba en `campaign_creators` —dos rondas por creador— y eso dejaba sin ninguna a las piezas siguientes en cuanto una se llevaba las dos. Dónde acaba la línea del cargo es `Q-57`. **Respaldada en el esquema desde 8.4**: `ck_cvw_round` exige que la ronda sea del cliente, `tg_cvw_techo` obliga a que `over_included` diga la verdad en las dos direcciones, y `tg_del_rondas` impide que el contador baje. Hasta entonces las tres vivían sólo en `Revisiones`. | 🟠 |
| **BR-CONTENT-004** | Una publicación no se considera verificada hasta que existe evidencia archivada del post en vivo, con fecha y hora de captura. **Implementada en 8.7** (`DEC-142`): `tg_pub_verificada_con_evidencia` impide pasar a `verified` sin un `screenshot` con archivo, y `ck_pev_screenshot` impide que una fila diga «captura» sin traer una. La evidencia lleva `no_delete` desde 3.12. **Una comprobación HTTP no cuenta**, y no es celo: Instagram y TikTok devuelven lo mismo para un post vivo que para un bloqueo, así que el estado no distingue una cosa de la otra. | 🔴 |
...
| **BR-CONTENT-006** | Si el contrato establece permanencia mínima del post, el sistema comprueba su disponibilidad y alerta ante una despublicación anticipada. **Implementada en 8.8**, y con lo que la regla no decía: retirar el post antes de tiempo **bloquea el pago** y no descuenta nada (`DEC-145`). La comprobación se **archiva** y no decide (`permanence_checks`, append-only); quien mueve una publicación a `removed` es una persona, y `tg_pub_permanencia` le exige una comprobación fallida **y** una captura posterior a la verificación. `permanencia:vigilar` cierra las ventanas cumplidas —`verified` → `fulfilled`, que es lo que habilita el pago— y cuenta las desatendidas; no sale a Internet. | 🟠 |
| **BR-CONTENT-010** | Un post sólo se registra si el entregable está **aprobado**, y se registra **qué versión** se publicó. Un mismo post no puede reclamarse desde dos entregables, y el enlace tiene que ser de la red que pide el brief. **Implementada en 8.6** (`DEC-140`, `DEC-141`): `tg_pub_version_aprobada`, `fk_pub_version` (compuesta), `uq_pub_fingerprint` sobre la URL **normalizada**, y `platforms.url_pattern` para reconocer la red. | 🟠 |
| **BR-CONTENT-008** | Un entregable sólo existe si hay un **compromiso aceptado** detrás, y sale del brief **efectivo** del mercado del creador, no del general. **Implementada en 8.1** (`DEC-129`, `DEC-130`): `tg_del_participacion_aceptada` lo impone en la base, y `tg_dv_participacion_viva` impide además añadir entregas sobre una participación `declined`, `expired` o `cancelled`. | 🟠 |
| **BR-CONTENT-009** | Si al texto de una entrega le faltan los hashtags o las menciones que exige el brief, **la entrega no se acepta**, y se le dice al creador cuáles faltan. **Implementada en 8.1** (`DEC-132`). La comparación no distingue mayúsculas, y se devuelven **todos** los motivos a la vez. | 🟡 |
| **BR-CONTENT-007** | Todo comentario de revisión queda asociado a una versión concreta. Un comentario nunca "flota" sobre el entregable. **Implementada en 8.3**: `content_reviews.deliverable_version_id` es obligatoria, `ck_cvw_comments` exige texto al pedir cambios, y `tg_cvw_ultima_version` obliga a que sea la **última** —un veredicto sobre contenido que el creador ya reemplazó no lo lee nadie—. Y `tg_cvw_inmutable` lo hace append-only de verdad: un veredicto no se edita, se emite otro. | 🟡 |
...
| **BR-FIN-004** | Todo importe se almacena junto a su moneda. Toda conversión registra monto origen, moneda origen, monto destino, moneda destino, tasa, fecha de la tasa y fuente. Está prohibido almacenar importes en punto flotante. *(Implementada en 9.1: `Cambio::convertir()` devuelve las siete y multiplica con `DECIMAL` en el motor, no con `float` ni con `bcmath`.)* | 🔴 |
...
| **BR-FIN-009** | El tipo de cambio aplicado a una operación es el vigente en la fecha de la operación, no el actual. Los históricos no se recalculan. *(Impuesta en 9.1: `tg_fx_inmutable` bloquea el `UPDATE` de una tasa publicada, `fx_official_sources` guarda con periodos quién mandaba en cada fecha, y un día sin tasa usa la anterior guardando **la fecha de esa tasa** (`DEC-160`).)* | 🔴 |
...
| **BR-FIN-018** | El régimen tributario se decide **por factura** y se congela junto al país del receptor **y al del emisor**. Una operación no gravada no lleva importe de impuesto, y no se exporta un servicio a un cliente domiciliado **en el país que emite** — que hasta `9.9b` decía «en Perú», con el país escrito dentro del esquema. | 🔴 |
...
| **BR-FIN-021** | **Un borrador de comprobante no gasta correlativo.** El número se pide en el instante de emitir, no al empezar a redactar: así descartar un borrador no abre un hueco en la numeración. Una vez emitido, serie, número y el número del libro (`document_numbers`) son obligatorios y únicos. | 🔴 |
| **BR-FIN-022** | **Un comprobante emitido no se corrige: se anula y se emite otro.** Su núcleo fiscal —importes, fechas, régimen, tasa, número y los datos congelados de las dos partes— queda inmutable desde ese instante. Lo único que sigue cambiando es su estado, el sello de la administración y el archivo. Impuesto por disparador, no por la capa de aplicación. | 🔴 |
| **BR-FIN-023** | **Las líneas de un comprobante suman su cabecera.** Se comprueba en el momento de emitir, y un comprobante sin líneas no se emite. | 🔴 |
| **BR-FIN-024** | Anular un comprobante exige un motivo escrito, y **el número no vuelve a la serie**: un comprobante anulado sigue existiendo ante la administración y sigue ocupando su correlativo. | 🔴 |
| **BR-FIN-025** | Cuál de los impuestos de un país va en una factura de venta lo dice la **configuración** (`countries.sales_tax_code`), no el código. Sin ese dato no se emite: el importe saldría en cero sin que nadie lo hubiera decidido. | 🔴 |
...
| **BR-SEC-003** | Un rol de ámbito externo (`client_user`, `creator`) nunca recibe permisos internos. **Reforzada en 5.9**, que creó la primera cuenta externa de verdad y destapó que la regla no bastaba: `/panel` no exigía ningún permiso —es la portada— y enseñaba los totales de creadores, clientes y campañas a cualquier autenticado. Un rol sin permisos no protege una pantalla que no pide ninguno. La portada se bifurca ahora por `user_type` (`DEC-118`). | 🔴 |
...
| **BR-LE-009** | *(Reformulada 2026-08-27 por `DEC-156`; ya no es «en el MVP».)* **La sociedad que factura una campaña paga a TODOS los creadores de esa campaña, sea cual sea el país de cada creador.** La sociedad se resuelve del país del CLIENTE (`BR-LE-003`) y se guarda en la campaña (`BR-LE-001`); el pago la hereda de ahí y la congela al confirmarse, igual que el importe. El país del creador determina **cómo** se le paga —retención, moneda, documento—, nunca **quién** le paga. Divergir requiere anulación explícita, autorizada y auditada. | 🔴 |
