...
| **BR-SEC-004** | Nunca se transmite una contraseña en texto claro por ningún canal. La activación de cuenta usa un enlace de un solo uso con expiración ≤72 h. **Implementada en 5.9/4.1 para las cuentas de creador**: la cuenta nace con un hash de 32 bytes aleatorios que nadie conoce, y la contraseña la escribe su dueño desde un enlace de 72 h (`DEC-113`) del que sólo se guarda la huella (`DEC-114`). ⚠️ **`T-36`: sigue sin cumplirse para los usuarios internos.** `usuarios:crear` teclea la contraseña y alguien la dicta — y son justo las cuentas que aprueban perfiles fiscales y verifican medios de pago, donde `ck_ctp_segregation` y `ck_cpm_segregation` suponen que dos `user_id` son dos personas. La pieza ya existe; falta llevarla al comando. | 🔴 |
