...
| **BR-CONTENT-004** | Una publicación no se considera verificada hasta que existe evidencia archivada del post en vivo, con fecha y hora de captura. **Media implementada en 8.6**: la publicación se registra y nace en `reported`; `ck_pub_verified` ya exigía quién y cuándo desde 2.12, y ahora `ck_pub_rejected` exige lo mismo al rechazarla. La evidencia archivada y el paso a `verified` son **8.7**. | 🔴 |
